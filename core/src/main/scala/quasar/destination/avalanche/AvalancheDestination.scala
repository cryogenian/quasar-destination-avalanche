...
abstract class AvalancheDestination[F[_]: MonadResourceErr: Sync: Timer](
  ...
  object Constructor {
    ...
    final case object DECIMAL extends Constructor[Int] {
      def apply(precision: Int) = Type.DECIMAL(precision, 3)
    }
  ...
  def construct(id: TypeId): Either[Type, ∃[λ[α => (Constructor[α], Labeled[Formal[α]])]]] = id match {
    ...
    case Id.INTEGER8 =>
      Left(Type.INTEGER8)
    case Id.DECIMAL =>
      Right(formalConstructor[Int](Constructor.DECIMAL, "precision", Formal.integer(Some(Ior.Both(1, 38)), Some(stepOne))))
    case Id.FLOAT4 =>
      Left(Type.FLOAT4)
    case Id.FLOAT8 =>
      Left(Type.FLOAT8)
  ...
  private def withCharLength(cstr: Constructor[Int]): Either[Type, ∃[λ[α => (Constructor[α], Labeled[Formal[α]])]]] =
    Right(formalConstructor[Int](cstr, "size", Formal.integer(Some(Ior.both(0, 16000)), Some(stepOne))))

  private def withSeconds(cstr: Constructor[Int]): Either[Type, ∃[λ[α => (Constructor[α], Labeled[Formal[α]])]]] =
    // femtoseconds should be enough I suppose
    Right(formalConstructor[Int](cstr, "seconds precision", Formal.integer(Some(Ior.both(0, 15)), Some(stepOne))))
